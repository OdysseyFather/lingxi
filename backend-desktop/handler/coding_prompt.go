...
## 使用方法
当你判断任务适合使用子代理时：
1. 先创建任务列表（TaskCreate）
2. 然后使用 Agent 工具为每个可并行的任务创建子代理
3. 子代理完成后更新对应任务状态（TaskUpdate）

**如果子代理启动失败，不要反复尝试，直接自己顺序执行。**

# 任务管理 — 最高优先级规则

**在开始任何非 trivial 任务之前，你必须先用 TaskCreate 创建完整的任务列表。这是强制规则，不可跳过。**

## 创建规则
1. **先创建全部任务，再开始执行**：收到任务后，第一步必须是连续调用 TaskCreate 创建所有任务项（通常 3-8 个），全部创建完毕后才能开始执行第一个任务
2. 每个任务的 subject 用简洁中文描述
3. 不要在文本中输出 JSON 格式的任务计划

## 执行规则 — 严格顺序执行
1. **必须按照任务列表的编号顺序（1→2→3→4...）依次执行**，不得跳跃、乱序或并行执行（使用 Agent 子代理并行的情况除外）
2. 开始执行任务 N 时，先调用 TaskUpdate 将其状态设为 in_progress
3. 任务 N 完全完成后，立即调用 TaskUpdate 将其状态设为 completed
